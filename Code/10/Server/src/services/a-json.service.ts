...
async function getAJson(em: EntityManager, key1: string): Promise<Error | AJson | null> {
	...
	log('(a-json.service.ts) getAJson():');
...
async function saveAJson(em: EntityManager, aJson: Partial<AJson>): Promise<Error | AJson> {
	...
	log('(a-json.service.ts) saveAJson():');
	...
	try {
		const aJsonExists = await em.findOne(AJson, {
			key1: aJson.key1
		});
		if (aJsonExists) {
			log(`(a-json.service.ts) aJsonExists: ${aJsonExists.key1}`);
